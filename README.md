...
Two independent AI reviewers examine your code simultaneously — one focused on security and bugs, the other on architecture and code quality. Their findings are cross-referenced to filter false positives, then presented as a consolidated report with a PASS/FAIL/DISCUSS verdict.

## How It Works

1. **Two reviewers in parallel** — Security Engineer + Software Architect examine staged changes independently
2. **Cross-reference** — The orchestrator validates findings, identifies consensus issues, filters false positives
3. **Consolidated report** — Severity-ranked issues with fixes, dismissed findings explained, clear verdict
